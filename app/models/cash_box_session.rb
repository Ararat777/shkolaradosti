class CashBoxSession < ApplicationRecord
  ...
  def make_cash_box_session_pdf
    report = self.build_report(:title => "Отчет кассовой смены №#{self.id}")
    report.path = "#{Rails.root}/app/reports/#{self.cash_box.branch.title}/CashBoxSessions"
  ...
  def make_income
    self.incomes.create(:amount => self.cash_box.cash,:title => "Остаток за #{self.cash_box.cash_box_sessions.where.not(:closed_at => nil).last.closed_at.strftime("%F")}")
  end
