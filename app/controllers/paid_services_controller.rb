class PaidServicesController < ApplicationController
  ...
  def update
    if @paid_service.update(:amount => (@paid_service.amount + paid_service_params[:amount].to_f), :end_date => paid_service_params[:end_date],:required_amount => paid_service_params[:required_amount])
      if @paid_service.amount > 0
